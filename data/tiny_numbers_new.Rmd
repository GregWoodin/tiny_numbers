...
```{r exclusion_plot, fig.width = 7.5, fig.height = 4.5}
...
reduc %>%
  ...
  geom_label(aes(label = videos), vjust = - 0.02, nudge_y = -250, colour = "black", fill = 'white', alpha = 1, show.legend = FALSE) +
  coord_flip() + 
  theme_classic() +
...
ggsave('exclusion_plot.pdf', width = 7.5, height = 4.5)

```

Also check how many videos there are in each dataset:
...
Check how many unique speakers there are in each dataset and overall:

```{r speakers}

# Tiny/huge numbers:
(size_length <- length(unique(size$SpeakerName)))

# Low/high numbers:
(height_length <- length(unique(height$SpeakerName)))

# Standards:
(valence_length <- length(unique(valence$SpeakerName)))

# Overall:
size_length + height_length + valence_length
...
ggsave('verticalmovement_adjstandards.pdf', width = 6.5, height = 4.5)
...
Looks like speakers were far more likely to gesture than not. Create plot showing gesture co-occurrence across each phrase (Figures 3a and 3b).

```{r occur_plot, fig.width = 10, fig.height = 6}

numbers <- filter(full, Phrase %in% c("tiny_number", "huge_number", "low_number", "high_number"))
standards <- filter(full, Phrase %in% c("low_standards", "high_standards", "lower_the_standards", "raise_the_standards"))

# Descriptive stats for 'numbers':
xtab_num <- table(numbers$Phrase, numbers$HandsMoving)    # Raw counts
props_num <- prop.table(xtab_num, 1)    # Row-wise percentages

# Descriptive stats for 'standards':
xtab_stan <- table(standards$Phrase, standards$HandsMoving)    # Raw counts
props_stan <- prop.table(xtab_stan, 1)    # Row-wise percentages

# Turn tables into dataframes and change column names:
props_num <- as.data.frame(props_num)   # Row-wise percentages
colnames(props_num) = c("Phrase", "Gesture", "Proportion")  
props_stan <- as.data.frame(props_stan)   # Row-wise percentages
colnames(props_stan) = c("Phrase", "Gesture", "Proportion")  

# Turn figures into numeric variables:
as.numeric(props_num$Proportion)  
as.numeric(props_stan$Proportion)  

# Re-order variables:
props_num$Phrase <- factor(props_num$Phrase, levels = c("tiny_number", "huge_number", "low_number", "high_number"))   # x-axis
props_num$Gesture <- factor(props_num$Gesture, levels = c("yes", "no"))    # Legend
props_stan$Phrase <- factor(props_stan$Phrase, levels = c("low_standards", "high_standards", "lower_the_standards", "raise_the_standards"))   # x-axis
props_stan$Gesture <- factor(props_stan$Gesture, levels = c("yes", "no"))    # Legend
...
plot1 <- 
  ...
  scale_fill_manual(values = c('forestgreen', 'grey75'), breaks = c("yes", "no"), name = "Gesture?", labels = c("Yes", "No")) +
  scale_x_discrete(labels = addline_format(c("tiny number", "huge number", "low number", "high number"))) +
  geom_label(aes(x = 0.7825, y = 0.93617021, label = "N = 88"), fill = "white", size = 4) +
  geom_label(aes(x = 1.225, y = 0.06382979, label = "N = 6"), fill = "white", size = 4) +
  geom_label(aes(x = 1.7825, y = 0.87234043, label = "N = 82"), fill = "white", size = 4) +
  geom_label(aes(x = 2.225, y = 0.12765957, label = "N = 12"), fill = "white", size = 4) +
  geom_label(aes(x = 2.7825, y = 0.72151899, label = "N = 57"), fill = "white", size = 4) +
  geom_label(aes(x = 3.225, y = 0.27848101, label = "N = 22"), fill = "white", size = 4) +
  geom_label(aes(x = 3.7825, y = 0.76470588, label = "N = 65"), fill = "white", size = 4) +
  geom_label(aes(x = 4.225, y = 0.23529412, label = "N = 20"), fill = "white", size = 4) 

ggsave('gesture_cooccurrence_nums.pdf', width = 10, height = 6)

# Make 'standards' graph
plot2 <- 
  props_stan %>%
  ggplot(aes(x = Phrase, y = Proportion, fill = factor(Gesture))) +
  geom_bar(stat = "identity", position = "dodge") +
  ...
  theme(axis.title.x = element_text(margin = margin(t = 15, r = 0, b = 0, l = 0))) +
  scale_fill_manual(values = c('forestgreen', 'grey75'), breaks = c("yes", "no"), name = "Gesture?", labels = c("Yes", "No")) +
  scale_x_discrete(labels = addline_format(c("low standards", "high standards", "lower the standards", "raise the standards"))) +
  geom_label(aes(x = 0.7825, y = 0.84693878, label = "N = 83"), fill = "white", size = 4) +
  geom_label(aes(x = 1.225, y = 0.15306122, label = "N = 15"), fill = "white", size = 4) +
  geom_label(aes(x = 1.7825, y = 0.79545455, label = "N = 70"), fill = "white", size = 4) +
  geom_label(aes(x = 2.225, y = 0.20454545, label = "N = 18"), fill = "white", size = 4) +
  geom_label(aes(x = 2.7825, y = 0.93518519, label = "N = 101"), fill = "white", size = 4) +
  geom_label(aes(x = 3.225, y = 0.06481481, label = "N = 7"), fill = "white", size = 4) +
  geom_label(aes(x = 3.7825, y = 0.92682927, label = "N = 76"), fill = "white", size = 4) +
  geom_label(aes(x = 4.225, y = 0.07317073, label = "N = 6"), fill = "white", size = 4)

ggsave('gesture_cooccurrence_stan.pdf', width = 10, height = 6)
